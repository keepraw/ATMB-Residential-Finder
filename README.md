...
- 支持多个API凭证，自动轮换使用
- 实时显示处理进度和结果统计
- **断点续传**：中途停止后重新运行可自动接续，无需从头开始
- **低速率请求**：每条详情页请求间隔1.5秒，大幅降低IP被封风险
...
程序支持断点续传，中途关闭（Ctrl+C 或重启电脑）后重新运行，会**自动跳过已处理的地址**，从上次停止的位置继续。
...
> ⚠️ **如果想从头重新开始**，请手动删除该文件：
> ```bash
> # Windows (命令提示符 / PowerShell)
> del result\progress.json
> 
> # macOS / Linux
> rm result/progress.json
> ```
> 程序检测不到 `progress.json` 时会自动从头抓取。

### 预计用时

目前 ATMB 全美约有 1700+ 个地址，程序每条请求间隔 1.5 秒（串行），完整跑完约需 **45 分钟**。
中途可随时关闭，下次运行会续上。
